...
func MigrateS3Deployment(cl client.Client, ns string) error {
	...
	err = migrate_s3_secret(cl, ns)
	if err != nil {
		return err
	}

	err = migrate_s3_system_app_dc(cl, ns)
	if err != nil {
		return err
	}

	err = migrate_s3_system_sidekiq_dc(cl, ns)
	if err != nil {
		return err
	}

	err = delete_unused_s3_data_configmap(cl, ns)
	if err != nil {
		return err
	}
...
func migrate_s3_secret(cl client.Client, ns string) error {
	existingSecret := &v1.Secret{}
	secretNamespacedName := types.NamespacedName{Name: "aws-auth", Namespace: ns}
	err := cl.Get(context.TODO(), secretNamespacedName, existingSecret)
	if err != nil {
...
func migrate_s3_system_app_dc(cl client.Client, ns string) error {
	changed := false
	...
	for _, envVar := range envVars {
		if !FindEnvByName(existingPreHookPod.Env, envVar) {
	...
	for _, container := range existing.Spec.Template.Spec.Containers {
		envVars := []string{"AWS_PROTOCOL", "AWS_HOSTNAME", "AWS_PATH_STYLE"}
		for _, envVar := range envVars {
			if !FindEnvByName(container.Env, envVar) {
				desiredEnvVar := FindContainerEnvByNameOrPanic(desiredContainers, container.Name, envVar)
...
func migrate_s3_system_sidekiq_dc(cl client.Client, ns string) error {
	changed := false
	existing := &appsv1.DeploymentConfig{}
...
func delete_unused_s3_data_configmap(cl client.Client, ns string) error {
	changed := false
